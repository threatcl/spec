...
> **Status:** Phase 2 (source-track hardening — committed artifacts) complete;
> the GitHub-settings checklist below is awaiting maintainer action. Phase 3
> pending.
...
- [x] **Phase 1** — Supply-chain hygiene: SHA-pinned actions in `testvet.yml`
  (`checkout` v4.3.1, `setup-go` v4.3.0), `.github/dependabot.yml`
  (`github-actions` + `gomod`), least-privilege `permissions: contents: read`.
...
- [ ] **Phase 3** — Releases + Build provenance (release workflow + checksums +
  `attest-build-provenance`), with verification docs.
...
> Filled in once the Phase 3 release workflow lands. Will document
> `gh attestation verify <artifact> --repo threatcl/spec` and `SHA256SUMS`
> checking so downstreams can verify a release.
