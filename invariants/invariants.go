...
//
// The package lives in the spec module so that every consumer — the threatcl
// CLI's `validate -invariants`, the Threatcl Cloud API — shares one parser and
// one evaluator, and so verdicts agree by construction rather than by two
// implementations being kept in step. It is a subpackage rather than part of
// package spec because invariants describe organisational policy over models,
// not the models themselves.
//
// Parsing (ParseFile, ParseHCLRaw) and evaluation (Evaluate) are the two seams;
// the cty mapping of spec structs is an internal concern. An Invariant is only
// constructible by parsing — its expression fields are deliberately unexported
// — so a rule that exists has been validated.
//
// The implementation was extracted from the threatcl CLI's internal/invariants
// package as of threatcl v0.6.5 (added in v0.6.0, commit f7a953d) with no
// behavioural change, so evaluation results are identical to those of the CLI
// before the move.
package invariants
...
// the waiver is auditable in the policy file. Reference is the source text of
// that model expression, captured at parse time so a caller can report the
// waiver the way its author wrote it.
...
// decode takes the whole *hcl.File, not just its body, because exemptions
// record the source text of their model expression and that needs f.Bytes.
func decode(f *hcl.File) ([]*Invariant, error) {
	var raw fileHCL
...
func (r *invariantHCL) validate(f *hcl.File) (*Invariant, error) {
	var errs []error
	...
	for i, e := range r.Exemptions {
		...
		exemptions = append(exemptions, &Exemption{
			...
			Reference:     exprSource(e.Model, f),
		})
	}
...
// exprSource returns the source text an expression was written as, so a
// reference can be reported back to the rule author verbatim.
func exprSource(expr hcl.Expression, f *hcl.File) string {
	if expr == nil || f == nil {
		return ""
	}
	return string(expr.Range().SliceBytes(f.Bytes))
}
